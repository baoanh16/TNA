<%@ Control Language="C#" AutoEventWireup="false" CodeBehind="OrderListControl.ascx.cs" Inherits="CanhCam.Web.ProductUI.OrderListControl" %>
...
<div class="admin-content col-md-12">
    ...
    <asp:Panel ID="pnlSearch" DefaultButton="btnSearch" CssClass="headInfo form-horizontal" runat="server">
        ...
        <div class="settingrow form-group">
            <gb:SiteLabel ID="lblddState" runat="server" Text="Loại đơn hàng"
                ForControl="ddStateID" CssClass="settinglabel control-label col-sm-3" />
            <div class="col-sm-9">
                <asp:DropDownList ID="ddStateID" runat="server">
                    <asp:ListItem Text="<%$Resources:ProductResources, OrderStatusAll %>" Value="-1"></asp:ListItem>
                    <asp:ListItem Text="Đơn hàng Affiliate" Value="-10"></asp:ListItem>
                    <asp:ListItem Text="Đơn hàng bình thường" Value="0"></asp:ListItem>
                    <asp:ListItem Text="Đơn hàng AccessTrade" Value="-20"></asp:ListItem>
                </asp:DropDownList>
            </div>
        </div>
        <div id="divOrderStatus" runat="server" class="settingrow form-group">
        ...
        <div class="settingrow form-group">
            <gb:SiteLabel ID="lblShippingStatus" runat="server" Text="Tình trạng giao hàng"
                ForControl="ddShippingStatus" CssClass="settinglabel control-label col-sm-3" />
            <div class="col-sm-9">
                <asp:DropDownList ID="ddShippingStatus" runat="server">
                    <asp:ListItem Text="<%$Resources:ProductResources, OrderStatusAll %>" Value="-1"></asp:ListItem>
                    <asp:ListItem Text="Chưa giao hàng" Value="0"></asp:ListItem>
                    <asp:ListItem Text="Đã giao chuyển phát" Value="1"></asp:ListItem>
                    <asp:ListItem Text="Đã giao cho khách hàng" Value="2"></asp:ListItem>
                </asp:DropDownList>
            </div>
        </div>
        <div class="settingrow form-group">
            ...
            <MasterTableView DataKeyNames="OrderId,OrderStatus" AllowSorting="false">
                <Columns>
                    ...
                    <telerik:GridTemplateColumn HeaderText="<%$Resources:ProductResources, OrderCreatedDateLabel %>">
                        <ItemTemplate>
                            <%# CanhCam.Web.ProductUI.ProductHelper.FormatDate(Convert.ToDateTime(Eval("CreatedUtc")), Resources.ProductResources.OrderCreatedDateFormat)%>
                        </ItemTemplate>
                    </telerik:GridTemplateColumn>
                    <telerik:GridTemplateColumn HeaderText="Trạng thái đơn hàng">
                        <ItemTemplate>
                    ...
                    <telerik:GridTemplateColumn HeaderText="Trạng thái giao hàng">
                        <ItemTemplate>
                            <%# CanhCam.Web.ProductUI.OrderHelper.GetShippingStatus(Convert.ToInt32(Eval("ShippingStatus"))) %>
                        </ItemTemplate>
                    </telerik:GridTemplateColumn>
                    <telerik:GridTemplateColumn HeaderText="Đơn hàng Affiliate?">
                        <ItemTemplate>
                            <asp:CheckBox ID="chkIsAffiliate" Enabled="false" CssClass="aff-checkbox" Checked='<%# Convert.ToInt32(Eval("StateId")) > 0 %>' runat="server" />
                            <asp:Literal ID="litAccessTrade" runat="server" Text='<%# Eval("CurrencyCode").ToString().Length > 0 ? "AT" : "" %>' />
                        </ItemTemplate>
                    </telerik:GridTemplateColumn>
                    ...
                    <telerik:GridTemplateColumn ItemStyle-HorizontalAlign="Right">
                        <ItemTemplate>
                            ...
                            <asp:HyperLink CssClass="cp-link" ID="EditLink" runat="server" 
                                Text="<%$Resources:ProductResources, OrderDetailLink %>" NavigateUrl='<%# SiteRoot + EditPageUrl + "?OrderID=" + Eval("OrderId") %>'>
                            </asp:HyperLink>
                        </ItemTemplate>
                    </telerik:GridTemplateColumn>
...
<style type="text/css">
    ...
    .OrderPopup .BillingDetails, .OrderPopup .ShippingDetails {width: 32%;}
    .OrderPopup .OrderProducts {width: 36%;}
    ...
    .aff-checkbox input {height:auto !important;}
</style>